...
def run_manage_py_command(subcommand, *args):
    env = get_env()
    cmd = ['docker-compose', '-f', DOCKER_COMPOSE_PATH, 'exec', 'app', './manage.py', subcommand] + list(args)
    subprocess.run(cmd, env=env)
...
def setup(args):
    ...
    run_manage_py_command('loaddata', 'ix_v2 code_v2 pirate_v1 wikipedia_v1 klarna_v1 bot_smith_v1')
...
def main():
    ...
    # 'down' subcommand
    parser_down = subparsers.add_parser('down', help='Stop and remove containers, networks, images, and volumes')
    ...
    # 'log' subcommand
    parser_log = subparsers.add_parser('log', help='View output from containers [worker, web, nginx, db, redis]')
